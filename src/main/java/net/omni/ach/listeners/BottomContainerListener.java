...
public class BottomContainerListener implements Listener {
    ...
    private void invalidateChainAbove(Block block) {
        if (!isContainerMat(block))
            return;

        Block current = block.getRelative(0, 1, 0);
        for (int i = 0; i < MAX_CHAIN_HEIGHT; i++) {
            if (tryInvalidateACH(current))
                return;

            for (int[] o : new int[][]{{1, 0}, {-1, 0}, {0, 1}, {0, -1}})
                if (tryInvalidateACH(current.getRelative(o[0], 0, o[1])))
                    return;
    ...
    private boolean tryInvalidateACH(Block block) {
